...
def svm_classifier(train_data, train_label, test_data, test_label, output_figure, plane, xx, yy, most_dominants, inverse_lengthscale):
    ...
    trainData = np.array( [train_data[:, most_dominants[0]]]).reshape(len(train_data), 1)
    testData = np.array( [test_data[:, most_dominants[0]]]).reshape(len(test_data), 1)

    # fit the model
    clf = svm.SVC( kernel=rbf_kernel, verbose=True)
    # clf = svm.SVC( verbose=True)
    # clf = svm.SVC( kernel='rbf', tol=1, verbose=True, gamma=inverse_lengthscale)
    clf.fit( trainData, train_label )

    test = clf.predict( trainData )
    print test[test==-1].size
...
def run_training(base_path, db_file, name_out_path, name):
    ...
    kern = GPy.kern.RBF( 1, ARD=True, lengthscale=lengthscale[most_dominants[0]])
